...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/frequency_sep2'), help='directory of the model')
    parser.add_argument('--model_name', default='com_model_144.pth', type=str, help='the model name')

    parser.add_argument('--decom_model_dir', default=os.path.join('../path_upgrade/low_threechannel'), help='directory of the model')
    parser.add_argument('--decom_model_name', default='model_038.pth', type=str, help='the model name')
...
if __name__ == '__main__':
    ...
    # model = DnCNN()
    decompose_model = DecomposeNet(image_channels=3, n_block=24)
    model = DecomposeNet(image_channels=5, n_block=32)
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                decompose_data = torch.cat([High_noise[:,1].unsqueeze(1), Low_noise], dim=1)
                ...
                dncnn_output = model(y_).squeeze(0)
                # dncnn_high, dncnn_low = Decomposition(dncnn_output, 0.125)

                # x_ = output[0].cpu().detach().numpy().astype(np.float32)

                output = ComplexTensor(dncnn_output[1] + dncnn_output[3], dncnn_output[2] + dncnn_output[4]).abs()
