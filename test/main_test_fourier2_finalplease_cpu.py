...
from model import Encoder1, DnCNN, Encoder2, NoBatchNorm, DecomposeNetLowFreq, DecomposeNet
...
def parse_args():
    ...
    # parser.add_argument('--low_model_dir', default=os.path.join('../path_upgrade/frequency_sep2'), help='directory of the model')
    # parser.add_argument('--low_model_name', default='com_model_129.pth', type=str, help='the model name')
    parser.add_argument('--low_model_dir', default=os.path.join('../path_upgrade/low_threechannel'), help='directory of the model')
    parser.add_argument('--low_model_name', default='model_042.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                #  0.14
                High_noise, Low_noise = Decomposition(y_.squeeze(0), 0.015)
                High_origin, Low_origin = Decomposition(torch.from_numpy(x).unsqueeze(0), 0.015)

                # y_ = torch.cat([y_,High_noise, Low_noise], dim=1)
                # x = torch.cat([High_origin.unsqueeze(1), Low_origin.unsqueeze(1)], dim=1)

                dncnn_input = High_noise[:,0].unsqueeze(1)
                ...
                lowfreq_input = torch.cat([High_noise[:, 1].unsqueeze(1), Low_noise[:, 0].unsqueeze(1),
                     Low_noise[:, 1].unsqueeze(1)], dim=1)

                output_dncnn = model_dncnn(dncnn_input.float()).squeeze(0) # inference
                ...
                output = ComplexTensor(output_dncnn[0] + lowfreq_output[0,1], lowfreq_output[0,0] + lowfreq_output[0,2]).abs()
                ...
                plt.figure()
                plt.imshow(output_dncnn[0].detach().numpy(), cmap='jet')
                plt.show()
                plt.close()
