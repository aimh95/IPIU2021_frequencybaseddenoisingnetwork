...
def Decomposition(input, end =0.125, start = 0.0):
    ...
    plt.show()
    #
    # imsave("../paper/frequency/high.png", np.clip(np.abs(np.real(np.fft.ifft2(High_freq)[0])), 0, 1))
    #
    # plt.figure()
    ...
    plt.show()
    #
    # imsave("../paper/frequency/low.png", np.clip(np.abs(np.real(np.fft.ifft2(Low_freq)[0])), 0, 1))
    #
    #
    # plt.figure()
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/unet_realcomplexsigma25'),
                        help='directory of the model')
    parser.add_argument('--model_name', default='model_1341.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                High_noise, Low_noise = Decomposition(torch.from_numpy(x).unsqueeze(0))
                High_noise, Low_noise = Decomposition(y_.squeeze(1))
                # _, _ = Decomposition(y_.squeeze(1)-torch.from_numpy(x).unsqueeze(0))
                ...
                # plt.figure()
                # plt.imshow(y_[0,0]-recon2, cmap='gray')
                # plt.show()
                # plt.close()
