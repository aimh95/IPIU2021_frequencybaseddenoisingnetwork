...
def parse_args():
    ...
    parser.add_argument('--model_name', default='model_048.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                y_ = y_.cuda()
                x_ = model(High_noise[:,0].unsqueeze(1).cuda())  # inference
                x_ = x_.view(y.shape[0], y.shape[1])
                x_ = x_.cpu()
                x_ = x_.detach().numpy().astype(np.float32)

                x = High_origin[0,0].detach().numpy().astype(np.float32)
