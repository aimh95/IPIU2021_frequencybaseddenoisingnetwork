...
def Decomposition(input, end =0.125, start = 0.0):
    ...
    plt.show()
    imsave("../paper/frequency.png",np.log10(np.abs(High_freq[0])))
    ...
    plt.figure()
    plt.imshow(np.log10(np.abs(Low_freq[0])), cmap='gray')
    plt.show()
    imsave("../paper/frequency_low.png",np.log10(np.abs(Low_freq[0]+1)))

    plt.figure()
    plt.imshow(input[0], cmap='gray')
    plt.show()
    imsave("../paper/input.png", input[0])

    plt.figure()
    plt.imshow(np.abs(np.real(np.fft.ifft2(High_freq)[0])), cmap='gray')
    plt.show()
    imsave("../paper/real_highfreq.png", np.abs(np.real(np.fft.ifft2(High_freq)[0])))

    plt.figure()
    plt.imshow(np.abs(np.imag(np.fft.ifft2(High_freq)[0])), cmap='gray')
    plt.show()
    imsave("../paper/imag_highfreq.png", np.abs(np.imag(np.fft.ifft2(High_freq)[0])))

    plt.figure()
    plt.imshow(np.abs(np.real(np.fft.ifft2(Low_freq)[0])), cmap='gray')
    plt.show()
    imsave("../paper/real_lowfreq.png", np.abs(np.real(np.fft.ifft2(Low_freq)[0])))

    plt.figure()
    plt.imshow(np.abs(np.imag(np.fft.ifft2(Low_freq)[0])), cmap='gray')
    plt.show()
    imsave("../paper/imag_lowfreq.png", np.abs(np.imag(np.fft.ifft2(Low_freq)[0])))

    plt.figure()
    plt.imshow(input[0]/3+np.abs(np.real(np.fft.ifft2(Low_freq)[0])*2/3), cmap='gray')
    plt.show()
    imsave("../paper/pre_process.png", input[0]/3+np.abs(np.real(np.fft.ifft2(Low_freq)[0])*2/3))
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/unet_realcomplexsigma15'),
                        help='directory of the model')
    parser.add_argument('--model_name', default='model_1837.pth', type=str, help='the model name')
