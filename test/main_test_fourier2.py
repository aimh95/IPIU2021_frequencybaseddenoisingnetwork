...
def Decomposition(input, end =0.2, start = 0.0):
    ...
    plt.figure()
    plt.imshow(High_output[0], cmap='jet')
    plt.show()

    plt.figure()
    plt.imshow(Low_output[0], cmap='jet')
    plt.show()

    plt.figure()
    plt.imshow(High_output[0]+Low_output[0], cmap='jet')
    plt.show()
...
def parse_args():
    ...
    parser.add_argument('--model_name', default='model_022225.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                High_noise, Low_noise = Decomposition(torch.tensor(y_).squeeze(1))
