...
def Reconstruct(high, low, end =0.36, start = 0.0):

    high = high.cpu().detach().numpy()
    High_freq = np.fft.fft2(high)
    ...
    output = np.zeros(High_freq.shape)
    ...
    for x in range(centerx-int(width*end/2), centerx+int(width*end/2)):
        for y in range(centery-int(height*end/2), centery+int(height*end/2)):
            output[:, x, y] = Low_freq[:, x, y]
            # output[:,x,y] = Low_freq[:,x,x]

    output = np.fft.ifftshift(output)
    output = np.fft.ifft2(output)

    plt.figure()
    plt.imshow(np.abs(output[0]), cmap='jet')
    plt.show()
...
def Decomposition(input, end =0.36, start = 0.0):
    ...
    # plt.figure()
    # plt.imshow(np.abs((np.fft.ifft2(Low_freq)[0])+ np.abs((np.fft.ifft2(High_freq)[0]))), cmap='jet')
    # plt.show()
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/edge_high_freq_final'), help='directory of the model')
    parser.add_argument('--model_name', default='model_031.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                Reconstruct(High_origin, Low_origin)
                ...
                plt.figure()
                plt.imshow(High_origin[0]+Low_origin[0], cmap='jet')
                plt.show()
                plt.close()
                ...
                y_ = y_.cuda()
                x_ = model(High_noise.unsqueeze(1).float().cuda())  # inference

                torch.cuda.synchronize()
                elapsed_time = time.time() - start_time
                # print('%10s : %10s : %2.4f second' % (set_cur, im, elapsed_time))

                x_, _ = Decomposition(x_.squeeze(0))
                ...
                plt.figure()
                plt.imshow(High_origin[0], cmap='jet')
                plt.show()
                plt.close()
                ...
                psnr_x_ = compare_psnr(High_origin[0].numpy(), x_)
                ssim_x_ = compare_ssim(High_origin[0].numpy(), x_)
