...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/dncnn_twochannel2'), help='directory of the model')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                decompose_data = torch.cat([y_, High_noise[:,0].unsqueeze(1)], dim=1)

                start_time = time.time()

                x_ = model(decompose_data)  # inference
                noise_level = activation['noise_level']
                # x_ = x_[:,1].view(y.shape[0], y.shape[1])
                x_ = x_.cpu()
                ...
                plt.figure()
                plt.imshow(x_[0,1], cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(x_[0,0], cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(noise_level[0,1], cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(y_[0,0]-noise_level[0,1], cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(y_[0,0]-noise_level[0,0], cmap='jet')
                plt.show()
                plt.close()

                psnr_x_ = compare_psnr(High_origin[0,0].numpy(), x_)
                ssim_x_ = compare_ssim(High_origin[0,0].numpy(), x_)
