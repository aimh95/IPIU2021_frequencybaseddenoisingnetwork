...
def parse_args():
    ...
    parser.add_argument('--three_model_dir', default=os.path.join('../path_upgrade/unet_realcomplex66'),
                        help='directory of the model')
    parser.add_argument('--three_model_name', default='model_036.pth', type=str, help='the model name')
...
if __name__ == '__main__':
    ...
    pre_model = DnCNN()
    # unet_model = UNetDense(input_channels=2, image_channels=1)
    unet_model = UNetDenseFrequencyVer6(input_channels=2, image_channels=1)
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                High_noise, Low_noise = Decomposition(y_, 0.125)

                start_time = time.time()
                pre_model = pre_model.cpu()
                output1 = pre_model(y_)
                ...
                # plt.show()
                # output = model(output1, Low_noise[:, :1])

                x_ = unet_model(output1.cpu(), Low_noise[:, :1].cpu())
