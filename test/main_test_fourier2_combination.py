    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                decom_output = decompose_model(decompose_data.cuda().float()).squeeze(0) # inference
                dncnn_output = model(dncnn_data).squeeze(0)

                # x_ = output[0].cpu().detach().numpy().astype(np.float32)
                output = ComplexTensor(decom_output[1] + decom_output[3], decom_output[2] + decom_output[4]).abs()
