...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/dncnn_twochannel'), help='directory of the model')
    parser.add_argument('--model_name', default='model_029.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                dncnn_input = torch.cat([y_, High_noise[:,0].unsqueeze(1)], dim=1)
                ...
                lowfreq_output = model_lowfreq(lowfreq_input).squeeze(0)


                # x_ = output[0].cpu().detach().numpy().astype(np.float32)
                # output = ComplexTensor(High_origin[0,0].cuda() + output[3], output[2] + output[4]).abs()
                output = ComplexTensor(output_dncnn[1] + lowfreq_output[1], lowfreq_output[0] + lowfreq_output[2]).abs()

                # output = output_dncnn[0]
                ...
                plt.figure()
                plt.imshow(dncnn_input[0,0], cmap='jet')
                plt.show()
                plt.close()
