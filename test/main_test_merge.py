...
def parse_args():
    ...
    parser.add_argument('--low_model_dir', default=os.path.join('../path_upgrade/edge_low_freq_20'), help='directory of the model')
    parser.add_argument('--low_model_name', default='model_017.pth', type=str, help='the model name')
    parser.add_argument('--res_model_dir', default='../path_upgrade/edge_low_merge_20_newstyle2', type=str,
                        help='path of train data')
    parser.add_argument('--res_model_name', default='model_010.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                x_ = res_model((y_.float()))
                for i in range(len(x_)):
                    x_[i] = x_[i] - torch.mean(x_[i])

                plt.figure()
                plt.imshow(High_origin[0].cpu().detach().numpy(), cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(High_noise[0].cpu().detach().numpy(), cmap='jet')
                plt.show()
                plt.close()
                ...
                plt.show()
                plt.close()
                test = x-low_[0,0].cpu().detach().numpy()
                plt.figure()
