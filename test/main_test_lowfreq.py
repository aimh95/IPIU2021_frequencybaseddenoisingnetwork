...
from model import Encoder1, DnCNN, Encoder2, NoBatchNorm, DecomposeNet, LowFreq
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/edge_low_freq_20'), help='directory of the model')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                y_ = y_.cuda()
                x_ = model(Low_noise.unsqueeze(1).float().cuda())  # inference

                plt.figure()
                plt.imshow(y_[0,0].cpu()-x_[0,0].cpu().detach().numpy(), cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow(temp-x_[0,0].cpu().detach().numpy(), cmap='jet')
                plt.show()
                plt.close()
                ...
                _, x_ = Decomposition(x_.squeeze(0))
                ...
                plt.figure()
                plt.imshow(Low_origin[0], cmap='jet')
                plt.show()
                plt.close()
                ...
                psnr_x_ = compare_psnr((Low_origin[0]/torch.max(Low_origin[0])).numpy(), x_/np.max(x_))
                ssim_x_ = compare_ssim((Low_origin[0].numpy()/torch.max(Low_origin[0])).numpy(), x_/np.max(x_))
                # psnr_x_ = compare_psnr((Low_origin[0]).numpy(), x_ )
                # ssim_x_ = compare_ssim(Low_origin[0].numpy(), x_ )
