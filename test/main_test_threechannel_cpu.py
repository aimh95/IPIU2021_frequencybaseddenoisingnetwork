...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../path_upgrade/low_threechannel2'), help='directory of the model')
    parser.add_argument('--model_name', default='model_052.pth', type=str, help='the model name')
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                decompose_data = torch.cat([High_noise[:,1].unsqueeze(1), Low_noise[:,0].unsqueeze(1), Low_noise[:,1].unsqueeze(1)], dim=1)

                start_time = time.time()

                x_ = model(decompose_data)  # inference
                x_ = x_[:,0].view(y.shape[0], y.shape[1])
                x_ = x_.cpu()
                ...
                plt.figure()
                plt.imshow((High_origin[0,1]), cmap='jet')
                plt.show()
                plt.close()

                plt.figure()
                plt.imshow((High_noise[0,1]), cmap='jet')
                plt.show()
                plt.close()
                ...
                psnr_x_ = compare_psnr(High_origin[0,1].numpy(), x_)
                ssim_x_ = compare_ssim(High_origin[0,1].numpy(), x_)

                # psnr_x_ = compare_psnr(High_origin[0, 1].numpy(), High_noise[0, 1].numpy())
                # ssim_x_ = compare_ssim(High_origin[0, 1].numpy(), High_noise[0, 1].numpy())
