...
from model import Encoder1, NoBatchNorm, DnCNN, DecomposeNet, DecomposeNetLowFreq
from loss import *
from utils import *
...
parser.add_argument('--save_dir', default='../path_upgrade/low_threechannel2', type=str, help='path of train data')
...
if __name__ == '__main__':
    ...
    model = DecomposeNetLowFreq(image_channels=3, n_block=18)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[50, 80, 120], gamma=0.2)  # learning rates
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            batch_y = torch.cat([High_noise[:,1].unsqueeze(1), Low_noise[:,0].unsqueeze(1), Low_noise[:,1].unsqueeze(1)], dim=1)
            batch_x = torch.cat([High_origin[:,1].unsqueeze(1), Low_origin[:,0].unsqueeze(1), Low_origin[:,1].unsqueeze(1)], dim=1)
