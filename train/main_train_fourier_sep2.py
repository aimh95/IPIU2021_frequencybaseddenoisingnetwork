...
if __name__ == '__main__':
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            if n_count % 800 == 0:
                highfreq1.imshow(High_noise[0,0].cpu(), cmap='gray')
                highfreq2.imshow(High_origin[0,0].cpu(), cmap='gray')
                highfreq3.imshow(decompose_output[0,1].cpu().detach().numpy(), cmap='gray')
                output1.imshow(temp[0].cpu(), cmap='gray')
                lowfreq1.imshow(Low_noise[0,0].cpu(), cmap='gray')
                lowfreq2.imshow(Low_origin[0,0].cpu(), cmap='gray')
                lowfreq3.imshow(temp2[0].cpu().detach().numpy(), cmap='gray')
                output2.imshow(decompose_output[0,0].squeeze(0).cpu().detach().numpy(), cmap='gray')
                plt.show()
