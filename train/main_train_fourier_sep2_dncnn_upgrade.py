...
parser.add_argument('--save_dir', default='../path_upgrade/frequency_sep2_dncnn_high', type=str, help='path of train data')
...
def Decomposition(input, start=0.0, end=1.0):
...
if __name__ == '__main__':
    ...
    if initial_epoch > 0:
        ...
        decompose_model.load_state_dict(torch.load(os.path.join(save_dir, 'decom_model_%03d.pth' % initial_epoch)))
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            if cuda:
                ...
                temp = batch_x
                temp2 = batch_y
                High_noise, Low_noise = Decomposition(batch_y, 0, 0.125)
                High_origin, Low_origin = Decomposition(batch_x, 0, 0.125)
            ...
            if n_count % 800 == 0:
                ...
                highfreq3.imshow(High_noise[0, 1].cpu().detach().numpy(), cmap='jet')
                output1.imshow(High_origin[0, 1].cpu(), cmap='jet')
                lowfreq1.imshow(Low_noise[0, 0].cpu(), cmap='jet')
                lowfreq2.imshow(Low_origin[0, 0].cpu(), cmap='jet')
                lowfreq3.imshow(Low_noise[0,1].cpu().detach().numpy(), cmap='jet')
                output2.imshow(Low_origin[0, 1].squeeze(0).cpu().detach().numpy(), cmap='jet')
                plt.show()
