...
from model import DecomposeNet, DecomposeNetLowFreq, DnCNN
from loss import *
from utils import *
...
parser.add_argument('--save_dir', default='../path_upgrade/frequency_sep2_decom_low', type=str, help='path of train data')
...
if __name__ == '__main__':
    ...
    # decompose_model = DnCNN(image_channels=3)
    decompose_model = DecomposeNetLowFreq(image_channels=3, n_block=24)
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            y_ = torch.cat([High_noise[:,1].unsqueeze(1).cuda(), Low_noise[:,0].unsqueeze(1).cuda(), Low_noise[:,1].unsqueeze(1).cuda()], dim=1)
            x_ = torch.cat([High_origin[:,1].unsqueeze(1).cuda(), High_origin[:,0].unsqueeze(1).cuda(), High_origin[:,1].unsqueeze(1).cuda()], dim=1)

            decompose_output = decompose_model(y_.cuda())
            decompose_loss = criterion(decompose_output, x_)
            loss = criterion(decompose_output[:,0], x_[:,0])
            ...
            highfreq2 = fig.add_subplot(gs[1, 0])
            highfreq3 = fig.add_subplot(gs[2, 0])

            lowfreq1 = fig.add_subplot(gs[0, 1])
            lowfreq2 = fig.add_subplot(gs[1, 1])
            lowfreq3 = fig.add_subplot(gs[2, 1])

            lowfreqimag1 = fig.add_subplot(gs[0, 2])
            lowfreqimag2 = fig.add_subplot(gs[1, 2])
            lowfreqimag3 = fig.add_subplot(gs[2, 2])
            ...
            lowfreqimag1.axis('off')
            lowfreqimag2.axis('off')
            lowfreqimag3.axis('off')

            if n_count % 800 == 0:
                highfreq1.imshow(High_noise[0, 1].cpu(), cmap='jet')
                highfreq2.imshow(High_origin[0, 1].cpu(), cmap='jet')
                highfreq3.imshow(decompose_output[0, 0].cpu().detach().numpy(), cmap='jet')

                lowfreq1.imshow(Low_noise[0, 0].cpu(), cmap='jet')
                lowfreq2.imshow(Low_origin[0, 0].cpu(), cmap='jet')
                lowfreq3.imshow(decompose_output[0, 1].cpu().detach().numpy(), cmap='jet')

                lowfreqimag1.imshow(Low_noise[0, 1].cpu(), cmap='jet')
                lowfreqimag2.imshow(Low_origin[0, 1].cpu(), cmap='jet')
                lowfreqimag3.imshow(decompose_output[0, 2].cpu().detach().numpy(), cmap='jet')
                plt.show()
            ...
            if n_count % 10 == 0:
                ...
                epoch + 1, n_count, xs.size(0) // batch_size, loss.item() / batch_size, decompose_loss.item() / batch_size))
