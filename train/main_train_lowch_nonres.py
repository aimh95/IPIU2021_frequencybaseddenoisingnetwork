...
from model import Encoder2, DnCNN, LowFreq
from loss import *
from utils import *
...
def Decomposition(input, end =0.2, start = 0.0):
    ...
    #
    # plt.figure()
    # plt.imshow(torch.tensor(input[0])-Low_output[0], cmap='jet')
    # plt.show()
...
parser.add_argument('--save_dir', default='../path_upgrade/edge_low_freq_20_nonres', type=str, help='path of train data')
...
if __name__ == '__main__':
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            batch_y = Low_noise.unsqueeze(1).cuda()
            batch_x = Low_origin.unsqueeze(1).cuda()
            ...
            if n_count == 0:
                ...
                res.imshow(temp2[0].cpu().squeeze(0) - output[0].cpu().detach().numpy().squeeze(0), cmap='rainbow')
                res2.imshow(temp1[0].cpu().squeeze(0) - output[0].cpu().detach().numpy().squeeze(0), cmap='rainbow')
