...
parser.add_argument('--save_dir', default='../path_upgrade/edge_low_merge_histo', type=str, help='path of train data')
parser.add_argument('--load_dir', default='../path_upgrade/edge_low_merge_final', type=str, help='path of train data')
...
if __name__ == '__main__':
    ...
    initial_epoch = findLastCheckpoint(save_dir=args.load_dir)  # load the last model in matconvnet style

    if initial_epoch > 0:
        print('resuming by loading epoch %03d' % initial_epoch)
        model.load_state_dict(torch.load(os.path.join(args.load_dir, 'model_%03d.pth' % initial_epoch)))
    ...
    intensity_criterion = Intensity_loss()
    ...
    if cuda:
        ...
        intensity_criterion = intensity_criterion.cuda()
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            loss2 = criterion(output+low_, temp.cuda())
            loss = intensity_criterion(output+low_,temp.cuda(),args.batch_size,8e-15)
            ...
            if n_count == 0:
                original.imshow(batch_x[0].cpu().detach().numpy().squeeze(0), cmap='jet')
                noised.imshow(batch_y[0].cpu().detach().numpy().squeeze(0), cmap='jet')
                out.imshow(output[0].cpu().detach().numpy().squeeze(0), cmap='jet')
                final.imshow((output+low_)[0].cpu().detach().numpy().squeeze(0), cmap='jet')
                original2.imshow(temp[0].cpu().detach().numpy().squeeze(0), cmap='jet')
                plt.show()
