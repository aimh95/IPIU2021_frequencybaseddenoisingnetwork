...
parser.add_argument('--save_dir', default='../path_upgrade/frequency_unet', type=str, help='path of train data')
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            if cuda:
                ...
                High_noise, Low_noise = Decomposition(batch_y, 6)
            ...
            # plot5.axis('off')
            if n_count == 0:
                plot1.imshow(torch.add(batch_x[:,0], batch_x[:,1])[0].cpu(), cmap='gray')
                plot2.imshow(torch.add(batch_y[:,0], batch_y[:,1])[0].cpu(), cmap='gray')
