...
parser.add_argument('--save_dir', default='../path_upgrade/dncnn_twochannel_unet', type=str, help='path of train data')

parser.add_argument('--load_model_dir', default=os.path.join('../path_upgrade/dncnn_twochannel2'),
                    help='directory of the model')
parser.add_argument('--load_model_name', default='model_086.pth', type=str, help='the model name')
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(os.path.join(args.load_model_dir, args.load_model_name)))

    initial_epoch = findLastCheckpoint(save_dir=save_dir)# load the last model in matconvnet style
    # initial_epoch = 150
    if initial_epoch > 0:
        print('resuming by loading epoch %03d' % initial_epoch)
        u_model.load_state_dict(torch.load(os.path.join(save_dir, 'model_%03d.pth' % initial_epoch)))
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            unet_input = torch.cat([batch_y, noise_level], dim=1)

            output = u_model(batch_y[:,:1])
