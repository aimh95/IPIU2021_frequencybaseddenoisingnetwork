...
from model import Encoder2, DnCNN, Model
from loss import *
from utils import *
...
parser.add_argument('--save_dir', default='../path_upgrade/edge_low_merge_20_newstyle2', type=str, help='path of train data')

parser.add_argument('--low_model_dir', default=os.path.join('../path_upgrade/edge_low_freq_20'),
                    help='directory of the model')
parser.add_argument('--low_model_name', default='model_031.pth', type=str, help='the model name')
...
if __name__ == '__main__':
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            # plt.figure()
            # plt.imshow(batch_x[0,0].cpu().detach().numpy(), cmap='jet')
            # plt.show()
            # plt.close()
            #
            # plt.figure()
            # plt.imshow(batch_y[0,0].cpu().detach().numpy(), cmap='jet')
            # plt.show()
            # plt.close()


            # for i in range(len(batch_y)):
            #     Edge_enhance[i] = normalize(batch_y[i].cpu())

            # plt.figure()
            # plt.imshow(Edge_enhance[0,0].cpu().detach().numpy(), cmap='jet')
            # plt.show()
            # plt.close()
