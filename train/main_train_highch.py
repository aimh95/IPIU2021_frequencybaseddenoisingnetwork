...
parser.add_argument('--save_dir', default='../path_upgrade/edge_high_freq_20', type=str, help='path of train data')
...
if __name__ == '__main__':
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        for n_count, batch_yx in enumerate(DLoader):
            ...
            org1 = fig.add_subplot(gs[1,0])
            org2 = fig.add_subplot(gs[1, 1])
            res1 = fig.add_subplot(gs[2, 0])
            res2 = fig.add_subplot(gs[2, 1])
            ...
            if n_count == 0:
                original.imshow(batch_x[0].cpu().squeeze(0), cmap='rainbow')
                noised.imshow(batch_y[0].cpu().squeeze(0), cmap='rainbow')
                out.imshow(output[0].cpu().detach().numpy().squeeze(0), cmap='rainbow')
                org1.imshow(temp2[0].cpu().squeeze(0) - output[0].cpu().detach().numpy().squeeze(0), cmap='rainbow')
                org2.imshow(temp1[0].cpu().squeeze(0) - output[0].cpu().detach().numpy().squeeze(0), cmap='rainbow')
                res1.imshow(temp2[0].cpu().squeeze(0), cmap='rainbow')
                res2.imshow(temp1[0].cpu().squeeze(0), cmap='rainbow')
